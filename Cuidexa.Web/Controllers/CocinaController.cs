...
public class CocinaController : Controller
{
    ...
    {
        var avisos = await _eventos.ObtenerParaEmpleadoAsync(RolEmpleado.Cocina, EmpleadoIdActual ?? 0);
        var turnosEspeciales = await _turnos.ObtenerTurnosEspecialesAsync(EmpleadoIdActual ?? 0);
        var comensalesHoy = await _comensales.ObtenerComensalesAsync(DateOnly.FromDateTime(DateTime.Today));

        ViewBag.AvisosPendientes = avisos.Count(e => !e.Leido);
        ViewBag.TurnosEspeciales = turnosEspeciales.Count;
        ViewBag.ComensalesHoy = comensalesHoy.TotalComensales;
        return View();
    }

    public async Task<IActionResult> Comensales(DateOnly? fecha)
    {
        return View(await _comensales.ObtenerComensalesAsync(fecha ?? DateOnly.FromDateTime(DateTime.Today)));
    }

    public async Task<IActionResult> Avisos()
    {
        return View(await _eventos.ObtenerParaEmpleadoAsync(RolEmpleado.Cocina, EmpleadoIdActual ?? 0));
    }
    ...
    {
        await _eventos.CrearAvisoManualAsync(residenteId, descripcion, RolEmpleado.Cocina, rolesAdicionales ?? new(), EmpleadoIdActual,
    ...
    public async Task<IActionResult> Turnos()
    {
        ...
        return View("~/Views/Shared/MisTurnos.cshtml", new MisTurnosViewModel
        {
            ...
            Companeros = await _turnos.ObtenerCompanerosAsync(RolEmpleado.Cocina, empleadoId),
    ...
    public async Task<IActionResult> CrearIncidencia(Models.Enums.TipoIncidencia tipo, int? residenteId, string titulo, string descripcion, Models.Enums.GravedadIncidencia gravedad, Models.Enums.CaracterIncidencia caracter)
    {
        try
        {
            await _incidencias.CrearAsync(tipo, residenteId, titulo, descripcion, gravedad, caracter, RolEmpleado.Cocina, EmpleadoIdActual);
